  ...
  const handleSearch = () => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter((item) => {
        const fieldValue = item[searchCategory]?.toLowerCase() || "";
    ...
    if (sortField) {
      ...
      filtered.sort((a, b) => {
        const valA = a[mappedField] || "";
        const valB = b[mappedField] || "";

        const isDate = sortField === "rental_date" || sortField === "return_date";
    ...
    if (window.confirm("정말 삭제하시겠습니까?")) {
      try {
        ...
        if (response.ok) {
          ...
          setFilteredData(filteredData.filter((item) => item.no !== bookId));
          alert("삭제가 완료되었습니다.");
        }
      } catch (error) {
